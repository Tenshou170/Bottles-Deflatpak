...
def test_recovery_smoke(manager):
    ...
    cur = con.cursor()
    cur.execute("SELECT status, ended_at, last_seen FROM sessions WHERE id=?", (1,))
...
def test_recovery_same_bottle_different_programs(manager):
    ...
    cur.execute(
        "SELECT COUNT(*) FROM sessions WHERE status='forced' AND bottle_id=?", ("b1",)
    )
    assert cur.fetchone()[0] == 2
...
def test_cli_manager_init_does_not_recover_live_sessions(
    temp_xdg_home, test_settings_stub
):
    ...
    try:
        manager = Manager(
            g_settings=test_settings_stub, check_connection=False, is_cli=True
        )
        tracker = manager.playtime_tracker
        assert tracker is not None
